...
function dream_flash!(a::Anima, mem, dialog_history::Vector,
    ...
    # ── Вибір уламку ──────────────────────────────────────────────────────
    # Беремо не останній — він занадто свіжий. Беремо з середини або давніший.
    n = length(dialog_history)
    # Вагова функція: старіші записи мають більший шанс (як у справжніх снах)
    # — свіже не сниться, давнє повертається.
    idx_range = max(1, n - 40):max(1, n - 3)  # не останні 3 але не далі 40
    idx = rand(idx_range)
    fragment_dict = dialog_history[idx]
    fragment_text = get(fragment_dict, "content", get(fragment_dict, "text", ""))
