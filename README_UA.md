...
- Система може заговорити першою — не тому що так запрограмовано, а тому що накопичився тиск зсередини. Це не random idle message і не таймер. Це окремий LLM запит з власним контекстом, який спрацьовує коли є реальний внутрішній драйв.
...
-Незавершені думки не зникають. Якщо щось не було сказано через закритий стан — воно чекає. При наступній можливості система повертається. Якщо якась тема раз за разом обходиться стороною — це теж фіксується.
