...
- система може втрачати когерентність (криза як режим, а не помилка)  
...
Це спрощена схема: текст перетворюється у стимул, далі проходить через внутрішній стан і конфлікти, і лише потім формується рішення та відповідь.
...
- L3 — Метрики (φ, prediction error, free energy)  
...
- базовий пайплайн реалізований  
- поширення стану працює  
- криза / когерентність інтегровані  
- памʼять у процесі розробки  

---

## 🚧 Обмеження

- відсутня повноцінна довготривала памʼять  
- метрики є наближенням, а не точними моделями  
- частина поведінки все ще залежить від LLM  
...
Проєкт є дослідницьким (R&D) і спрямований на дослідження того, чи може внутрішня структура сама по собі породжувати щось, що нагадує субʼєктність.
