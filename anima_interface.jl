...
function repl!(
    ...
    println(
        "  :save :state :vfe :blanket :hb :gravity :anchor :solom :self :crisis :history :clearhist :quit",
    )
    println("═"^70*"\n")
    dialog_path = replace(a.psyche_mem_path, "psyche" => "dialog")
    history = dialog_load(dialog_path)
    !isempty(history) &&
        println("  [DIALOG] Завантажено $(length(history)) реплік з $(dialog_path)\n")
    pending_llm = nothing
    pending_user_msg = ""
    ...
    while true
        ...
        elseif cmd==":state"
            ...
            _vad_s=to_vad(a.nt);
            _vfe_s=compute_vfe(a.gen_model, _vad_s)
            _phi_s=compute_phi_posterior(
                a.iit,
                _vad_s,
                a.sbg.epistemic_trust,
                a.blanket.integrity,
            ...
            println(
                "  ♥ $(round(60000.0/a.heartbeat.period_ms,digits=0))bpm  HRV=$(round(a.heartbeat.hrv,digits=3))  coh=$(round(a.crisis.coherence,digits=3))",
            )
            println(
                "  Тіло: $(build_inner_voice(a.body, a.nt, Int(a.crisis.current_mode), _phi_s, a.flash_count))",
            )
            println(
        ...
        elseif cmd==":hb"
            ...
            println(
                "  Симпатична=$(round(hb.sympathetic_tone,digits=3)) Парасимп=$(round(hb.parasympathetic_tone,digits=3))",
            )
        ...
        elseif cmd==":self"
            ...
            println("\n  Self-Belief Graph:")
            println(
                "  Attractor stability=$(round(sbg.attractor_stability,digits=3))  Epistemic trust=$(round(sbg.epistemic_trust,digits=3))",
            )
            ...
            for (name, b) in sort(collect(sbg.beliefs), by = kv->-kv[2].centrality)
                status =
                    b.confidence < 0.15 ? "💀collapsed" :
                    b.confidence < 0.35 ? "⚠️pressure" : "✓"
                @printf(
            ...
            println(
                "  Agency confidence=$(round(a.agency.agency_confidence,digits=3))  causal_ownership=$(round(a.agency.causal_ownership,digits=3))\n",
            )
        ...
        elseif cmd==":crisis"
            ...
            println("\n  Crisis Monitor:")
            println(
                "  Mode: $(cs.mode_name)  Coherence=$(cs.coherence)  Steps in mode=$(cs.steps_in_mode)",
            )
            println("  $(cs.note)")
